...
int weight(int i, int j) {
    //if (n_cities>32000)
    if (n_cities > 15000)
        return round(sqrt(pow(coord_x[i] - coord_x[j], 2) +
                          pow(coord_y[i] - coord_y[j], 2)));        // compute the distance between the cities i and j
    else
        return W[i][j];
}
...
int
d4_vertices_id(int *solution_blue, int *solution_red, int *d4_vertices, int *common_edges_blue, int *common_edges_red) {
...
void simplifyPaths(int *solution_blue_p2, int n_new, int *vector_comp, int *vector_cand, int *n_entries, int n_cand) {
    ...
    comp_seq = aloc_vectori(
            n_new);            // sequence of components for all entries/exits in unfeasible components in the order given by sol_blue
...
void tourTable_fill(int **Tour_table, int *d2_vertices, int *solution_blue_p2, int *solution_red_p2, int *solution_red,
                    int *label_list, int *label_list_inv, int *common_edges_blue_p2, int *common_edges_red_p2,
                    int n_new) {
...
void tourTable(int *solution_blue_p2, int *solution_red_p2, int *solution_red, int *label_list, int *label_list_inv,
    ...
    recently_assigned = aloc_vectori(
            n_new);            // indicates the recently assigned nodes (for reversing ghost nodes)
    entries_flag_rev = aloc_vectori(
            n_new);                // auxiliary vector used for checking direction of the entries
    ...
    d2_vertices_id(d2_vertices, solution_blue_p2, common_edges_blue_p2,
                   n_new);            // identifying the vertices with degree 2
    tourTable_fill(Tour_table, d2_vertices, solution_blue_p2, solution_red_p2, solution_red, label_list, label_list_inv,
                   common_edges_blue_p2, common_edges_red_p2, n_new);        // filling col. 0-3 of the tours table
    ...
    do {
        ...
        simplifyPaths(solution_blue_p2, n_new, vector_comp, vector_cand_dir, n_entries_dir,
                      cand_dir);    // correcting the number of entries (removing common paths and assigned components)
        ...
        simplifyPaths(solution_blue_p2, n_new, vector_comp, vector_cand_rev, n_entries_rev,
                      cand_rev);    // correcting the number of entries (removing common paths and assigned components)
...
double gpx(int *solution_blue, int *solution_red, int *offspring) {
    ...
    tourTable(solution_blue_p2, solution_red_p2, solution_red, label_list, label_list_inv, vector_comp, n_new,
              common_edges_p2_blue, common_edges_p2_red); // identify connected comp. using tour table
    ...
    candidate->fusion(solution_blue_p2,
                      solution_red_p2);            // if candidate i did not pass the test and has conditions, apply fusion with the neighbour with more connections
    candidate->fusion(solution_blue_p2,
                      solution_red_p2);            // if candidate i did not pass the test and has conditions, apply fusion with the neighbour with more connections
    candidate->fusion(solution_blue_p2,
                      solution_red_p2);            // if candidate i did not pass the test and has conditions, apply fusion with the neighbour with more connections
    ...
    candidate->fusionB(solution_blue_p2,
                       solution_red_p2);            // if candidate i did not pass the test and has conditions, apply fusionB to find fusions of partitions in order to have partitions with 2 cutting points
    //candidate->print();			// print the components
